...
class Survey(NamedModel):
    ...
    start_date = Column(TEXT, TIMESTAMP(timezone=True), nullable=False)
    end_date = Column(TEXT, TIMESTAMP(timezone=True), nullable=False)
    type_id = Column(UUID(as_uuid=True), ForeignKey("survey_types.id"))
    juisdiction_id = Column(UUID(as_uuid=True), ForeignKey("jurisdictions.id"))
