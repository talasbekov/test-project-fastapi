...
router = APIRouter(prefix="/academic_degrees", tags=["Institutions"], dependencies=[Depends(HTTPBearer())])
...
async def get_all(*,
    ...
    """
        ...
        - **skip**: int - The number of Institutions to skip before returning the results. This parameter is optional and defaults to 0.
        - **limit**: int - The maximum number of Institutions to return in the response. This parameter is optional and defaults to 100.
    """
...
async def help(*,
    db: Session = Depends(get_db)):
    institution_service.add_Institution(db, InstitutionCreate(name="test", url="sad"))
