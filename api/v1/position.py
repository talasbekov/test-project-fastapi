...
@router.post("", status_code=status.HTTP_201_CREATED, response_model=List[PositionRead],
...
@router.put("/{id}/", response_model=List[PositionRead], dependencies=[Depends(HTTPBearer())])
async def update(*,
    db: Session = Depends(get_db),
    id: str,
    body: PositionUpdate,
...
@router.delete("/{id}/", status_code=status.HTTP_202_ACCEPTED, response_model=List[PositionRead],
               dependencies=[Depends(HTTPBearer())])
async def delete(*,
    db: Session = Depends(get_db),
    id: str,
