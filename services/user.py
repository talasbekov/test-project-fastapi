...
class UserService(ServiceBase[User, UserCreate, UserUpdate]):
    ...
    def get_all(
        ...
        if hr_document_template_id is not None:
            ...
            users = (
                self._filter_for_eligible_actions(db, users, hr_document_template_id)
                .except_(excepted_users)
                .filter(self.model.is_active == True)
            )

            if template.subject_type == SubjectType.CANDIDATE.value:
                candidate_staff_division = staff_division_service.get_by_name(
                    db, StaffDivisionEnum.CANDIDATES.value
                )
                users = users.join(
                    StaffUnit, StaffUnit.id == self.model.staff_unit_id
                ).filter(StaffUnit.staff_division_id == candidate_staff_division.id)
        total = users.count()
        users = (
            users.order_by(func.to_char(User.last_name))
            .distinct()
            .offset(skip)
            .limit(limit)
            .all()
    ...
    def is_template_accessible_for_user(
    ...
    def get_all_active(
        ...
        view_all_emplayees = int(PermissionEnum.VIEW_ALL_EMPLOYEES.value)
        view_department_emplayees = int(PermissionEnum.VIEW_ALL_EMPLOYEES.value)
        if view_all_emplayees in permissions:
            has_permission = permission_service.has_permission(
                db, user_id, role, permissions, view_all_emplayees
            )
            if not has_permission:
                raise ForbiddenException("У вас нет требуемого разрешения для просмотра всего списка.")

            user_queue = self._get_users_by_filter_is_active(db, filter, True, user_id)
        elif view_department_emplayees in permissions:
            has_permission = permission_service.has_permission(
                db, user_id, role, permissions, view_department_emplayees
            )
            if not has_permission:
                raise ForbiddenException("У вас нет требуемого разрешения для просмотра списка департамента.")

            user_queue = self._get_users_by_filter_is_active_by_division(db, filter, True, user_id, role)
        else:
            raise ForbiddenException("У вас нет требуемого разрешения.")

        users = (
            user_queue.order_by(self.model.last_name.asc())
            .offset(skip)
            .limit(limit)
            .all()
        )
        for user in users:
            status_histories = (
                ...
                .filter(StatusHistory.user_id == user.id)
                .all()
            )
            for status_history in status_histories:
                if status_history != [] and (status_history.date_to is not None):
                    ...
                    if (
                            status_history.date_from < today
                            and status_history.date_to > today
                    ):
    ...
    def get_all_archived(
        self, db: Session, filter: str, skip: int, limit: int, user_id: str
    ):
        user_queue = self._get_users_by_filter_is_active(db, filter, False, user_id)
    ...
    def update_user_patch(self, db: Session, id: str, body: UserUpdate) -> User:
        ...
        if body.icon is not None:
            user.icon = body.icon
        if (
            body.call_sign is not None
            and body.call_sign.lower() != user.call_sign.lower()
        ):
            # self._validate_call_sign(db, body.call_sign)
            self._swipe_call_sign(
                db, user.call_sign.capitalize(), body.call_sign.capitalize()
            )
    ...
    def get_all_by_staff_unit(self, db: Session, staff_unit_id: str):
        ...
        users = (
            db.query(self.model).filter(self.model.staff_unit_id == staff_unit_id).all()
        )
    ...
    def get_user_by_staff_unit(self, db: Session, staff_unit_id):
        ...
        users = (
            db.query(self.model)
            .filter(self.model.staff_unit_id == staff_unit_id)
            .first()
        )

        return users

    def get_by_schedule_id(self, db: Session, schedule_id: str, skip: int, limit: int):
    ...
    def get_by_iin(self, db: Session, iin: str):
        ...
        user = db.query(self.model).filter(self.model.iin == iin).first()
    ...
    def get_by_jurisdiction(
        ...
        for i in jurisdictions:
            ...
            if i.name == JurisdictionEnum.PERSONNEL.value:
                return self.get_users_by_staff_division(db, staff_division)

            if i.name == JurisdictionEnum.SUPERVISED_EMPLOYEES.value:
                return (
                    db.query(self.model)
                    .filter(self.model.supervised_by.isnot(None))
                    .all()
                )
    ...
    def get_users_by_staff_division(
        self, db: Session, staff_division: StaffDivision
    ) -> List[User]:
        # Получаем все дочерние штатные группы пользователя, включая саму
        # группу
        staff_divisions: List[StaffDivision] = (
            staff_division_service.get_all_child_groups(db, staff_division.id)
        )
        ...
        for i in staff_divisions:
            staff_units.extend(
                staff_unit_service.get_all_staff_units_by_staff_division_id(db, i.id)[
                    "objects"
                ]
            )
    ...
    def _get_excepted_users_by_document_in_progress(
        self, db: Session, hr_document_template_id: str
    ):
        ...
        for step in steps:
            if step.is_direct_supervisor:
                excepted_users = (
                    excepted_users.join(StaffUnit, StaffUnit.id == User.staff_unit_id)
                    .join(
                        StaffDivision, StaffDivision.id == StaffUnit.staff_division_id
                    )
                    .filter(StaffDivision.leader_id.is_(None))
                )
                break
        excepted_users = excepted_users.filter(
    ...
    def _get_users_by_combat_unit_jurisdiction(self, db: Session):
        staff_divisions = (
            db.query(StaffDivision).filter(StaffDivision.is_combat_unit == True).all()
        )
    ...
    def _get_users_by_staff_unit_jurisdiction(self, db: Session):
        staff_divisions = (
            db.query(StaffDivision).filter(StaffDivision.is_combat_unit == False).all()
        )
    ...
    def _get_users_by_candidates_jurisdiction(self, db: Session):
        staff_division = (
            ...
            .filter(StaffDivision.name == StaffDivisionEnum.CANDIDATES.value)
            .first()
        )
    ...
    def _get_users_by_filter_is_active(
        self, db: Session, filter: Optional[str], is_active: bool, user_id: str
    ) -> Query[Any]:
        users = db.query(self.model).filter(
            self.model.is_active == is_active, self.model.id != user_id
        )
    ...
    def _get_users_by_filter_is_active_by_division(
        self,
        db: Session,
        filter: Optional[str],
        is_active: bool,
        ...
        staff_unit_active: StaffUnit = staff_unit_service.get_by_id(db, role)
        print(staff_unit_active.staff_division_id)
        # users = db.query(self.model).filter(
        #     self.model.is_active == is_active,
        #     self.model.id != user_id,
        #     StaffUnit.staff_division_id == staff_unit_active.staff_division_id
        # )
        users = db.query(self.model) \
    ...
    def _add_filter_to_query_(self, user_query, filter):
        key_words = filter.lower().split()
        # Предполагаем, что есть дополнительное поле 'full_name_lower' в модели User
        conditions = [User.full_name_lower.contains(name) for name in key_words]
        users = user_query.filter(or_(*conditions))  # Используйте or_ для более гибкого поиска
        return users
    ...
    #             func.concat(
    #                 func.concat(
    #                     func.concat(func.lower(User.first_name), " "),
    ...
    def _filter_for_eligible_actions(
        self, db: Session, user_query: Query[Any], hr_document_template_id: str
    ):
        from .constructor import handlers

        template = hr_document_template_service.get_by_id(db, hr_document_template_id)
    ...
    def get_all_iin_by_ids(
        self, db: Session, user_ids: List[str], candidate_ids: List[str]
    ):
    ...
    def get_leader_id(self, db: Session, staff_division_id: str) -> Optional[str]:
        staff_division = (
            db.query(StaffDivision)
            .filter(StaffDivision.id == staff_division_id)
            .first()
        )
    ...
    def get_all_unemployed(
        self, db: Session, is_free: bool, skip: int, limit: int
    ) -> List[UserShortRead]:
        if is_free:
            users = (
                db.query(User)
