...
class HospitalDataService(ServiceBase[HospitalData,HospitalDataCreate,HospitalDataUpdate]):
    def get_by_id(self,db: Session,id: str):
        hospital_data = super().get(db,id)
        if hospital_data is None:
            raise client.NotFoundException(detail="Hospital data is not found!")
