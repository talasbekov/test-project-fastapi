...
class SecondmentService(
    ...
    def get_by_option(self, db: Session, type: str,
        ...
        return [StaffDivisionOptionRead.from_orm(
            i) for i in staff_division_service.get_child_groups(db, id, skip, limit)]

    def get_object(self, db: Session, value: str, type: str):
        if is_valid_uuid(value):
            return db.query(StaffDivision).filter(
    ...
    def stop_relation(self, db: Session, user_id: uuid.UUID, id: uuid.UUID):
        db.query(SecondmentHistory).filter(SecondmentHistory.secondment_id == id).update(
            {SecondmentHistory.date_to: datetime.now()})
