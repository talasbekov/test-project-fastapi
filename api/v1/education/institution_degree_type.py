...
from schemas.education import InstitutionDegreeTypeCreate, InstitutionDegreeTypeRead, InstitutionDegreeTypeUpdate
from services.education import institution_degree_type_service

router = APIRouter(prefix="/institution_degree_types", tags=["InstitutionDegreeTypes"], dependencies=[Depends(HTTPBearer())])
...
async def get_all(*,
    ...
    """
        ...
        - **skip**: int - The number of InstitutionDegreeTypes to skip before returning the results. This parameter is optional and defaults to 0.
        - **limit**: int - The maximum number of InstitutionDegreeTypes to return in the response. This parameter is optional and defaults to 100.
    """
...
async def get_by_id(*,
    ...
    """
    Authorize.jwt_required()
    return institution_degree_type_service.get_by_id(db, id)
...
async def update(*,
    ...
    return institution_degree_type_service.update(
        db,
        db_obj=institution_degree_type_service.get_by_id(db, id),
        obj_in=body)
...
async def delete(*,
    ...
    """
    Authorize.jwt_required()
    institution_degree_type_service.remove(db, id)


@router.get("/help")
async def help(*,
    db: Session = Depends(get_db)):
    institution_degree_type_service.add_InstitutionDegreeType(db, InstitutionDegreeTypeCreate(name="test", url="sad"))
